...
from modules.model import ENCODER_ONLY_MODEL, DECODER_ONLY_MODEL, ENCODER_DECODER_MODEL

from config.base import BaseConfig
from modules.block import build_embedding_block, build_encoder_block, build_decoder_block
...
class TransformerDecoderOnlyModel(paddle.nn.Layer):
    ...
    def __init__(self, config: TransformerDecoderOnlyModelConfig):
        ...
        self.embed = build_embedding_block(config=config.embed_config)
        ...
        if config.ln_config is not None:
            self.ln_f = build_norm(config=config.ln_config)
    ...
    def forward(
        ...
        if self.config.perform_linear_bias:
            if attention_mask_2d is None:
                attention_mask_2d = paddle.ones(hidden_states.shape[:2], dtype=paddle.float32)

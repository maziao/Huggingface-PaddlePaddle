...
class OPTLearnedPositionalEmbedding(paddle.nn.Embedding):
    ...
    def forward(self, attention_mask: paddle.Tensor, past_key_values_length: int = 0):
...
@EMBEDDING.register_module
class OPTEmbedding(paddle.nn.Layer):
    config_class = TransformerEmbeddingConfig
    ...
    def __init__(self, config: TransformerEmbeddingConfig):
        ...
        self.token_embed = paddle.nn.Embedding(num_embeddings=config.n_vocab, embedding_dim=config.n_embed)
        if config.n_pos != 0:
            self.pos_embed = OPTLearnedPositionalEmbedding(num_embeddings=config.n_pos, embedding_dim=config.n_embed)
        if config.n_token_type != 0:
            self.type_embed = paddle.nn.Embedding(num_embeddings=config.n_token_type, embedding_dim=config.n_embed)
        if config.do_ln:
            self.ln = paddle.nn.LayerNorm(normalized_shape=config.n_embed, epsilon=config.ln_eps)
    ...
    def forward(
        ...
        if hasattr(self, 'pos_embed'):
            if position_ids is None:
                position_ids = paddle.ones([input_ids.shape[0], input_ids.shape[1] + past_length])
            hidden_states += self.pos_embed(position_ids, past_length)

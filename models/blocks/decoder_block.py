...
from models.registry import DECODER_BLOCK, build_attention, build_mlp
...
class TransformerDecoderBlock(paddle.nn.Layer):
    ...
    def __init__(self, config: TransformerDecoderBlockConfig):
        super().__init__()
        self.config = config
        self.ln_1 = paddle.nn.LayerNorm(normalized_shape=config.n_embed, epsilon=config.ln_eps)
        self.self_attn = build_attention(config=config.attn_config)
        if config.add_cross_attn:
            self.ln_cross_attn = paddle.nn.LayerNorm(normalized_shape=config.n_embed, epsilon=config.ln_eps)
            self.cross_attn = build_attention(config=config.attn_config)
        self.ln_2 = paddle.nn.LayerNorm(normalized_shape=config.n_embed, epsilon=config.ln_eps)

...
class DisjunctiveConstraint(Constraint):
    ...
    def __init__(self, nested_token_ids: List[List[int]]):
        super(Constraint, self).__init__()
        if not isinstance(nested_token_ids, list) or len(nested_token_ids
            ) == 0:
            raise ValueError(
        ...
        if any(any(not isinstance(token_id, int) or token_id < 0 for
            token_id in token_ids) for token_ids in nested_token_ids):
            raise ValueError(
...
class ConstraintListState:
    ...
    def init_state(self):
        ...
        self.pending_constraints = [constraint.copy(stateful=False) for
        ...
        if self.inprogress_constraint is not None:
            ...
            for cidx, pending_constraint in enumerate(self.pending_constraints):
                if pending_constraint.does_advance(token_id):
                    ...
                    if not stepped:
                        raise Exception(
                            '`constraint.update(token_id)` is not yielding incremental progress, even though `constraint.does_advance(token_id)` is true.'
                            )
                    if complete:
                    ...
                    if complete or stepped:
                        self.pending_constraints = self.pending_constraints[:
                            cidx] + self.pending_constraints[cidx + 1:]
                        if len(self.pending_constraints
                            ) == 0 and self.inprogress_constraint is None:
                            self.completed = True
                        break
        return complete, stepped
    ...
    def copy(self, stateful=True):
        ...
        if stateful:
            new_state.complete_constraints = [constraint.copy(stateful=True
                ) for constraint in self.complete_constraints]
            if self.inprogress_constraint is not None:
                new_state.inprogress_constraint = (self.
                    inprogress_constraint.copy(stateful=True))
            new_state.pending_constraints = [constraint.copy() for
                constraint in self.pending_constraints]
        return new_state
